...
- 実装は subagent に出す (superpowers の subagent-driven-development)。実装と修正の subagent は Opus (`model: opus`)、レビューと調査は review-perspectives の表のモデル。model は毎回明示する (省略すると親のモデルを継ぐ)。
