...
制約かどうかは反事実で決める: コメント無しで有能な読者がコードに加える変更 (行を消す、簡単な形にする、既定値に戻す) を名指しし、コメントの各文について「その文が偽ならその変更をしてよいか」を問う。よいなら制約。外部システムの挙動の事実でも、「〜ので」の形でも同じ。
...
- 外部システムの観測可能な挙動 (OS・デーモン・API の振る舞い等) を制約にしたコメントは、その挙動を落ちるテストで固定できるなら固定もする。コメントは陳腐化しても落ちないが、テストは落ちる。制約でない外部の挙動の解説は書かない。
