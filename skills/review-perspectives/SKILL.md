...
5. まとめた指摘を判定つきで報告する。起動しなかった担当とその理由も書く。
   - 反証した指摘は、作業ディレクトリの `exclusions.md` に次を書き足す。同じ対象のレビューと修正を繰り返す間、手順 3 で同じ指摘を検証し直さずに外すため。
     - 箇所・主張・反証の根拠。
     - 箇所のファイルと、根拠に引用した全ファイル (呼び出し元・呼び出し先・型定義・ガード) の `git -C <repo> ls-tree <tree=> -- <ファイル>` の行 (モードと blob id)。`<repo>` と `<tree=>` は手順 1 の出力。対象を指定したときはチェックアウトと別の tree なので、cwd のファイルを hash しない。
     - 手順 1 の出力の `rules=`。
