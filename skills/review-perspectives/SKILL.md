...
5. まとめた指摘を判定つきで報告する。起動しなかった担当とその理由も書く。
   ...
   - コードの挙動についての指摘 (手順 3 で検証エージェントに渡したもの) のうち確定・あり得ると判定したものは、作業ディレクトリの `findings.md` に「箇所 (ファイルと関数か節)・観点・回・`tree=`」を 1 行で書き足す。回は、`findings.md` にある今回と違う `tree=` の種類の数 + 1。同じ `tree=` の行は同じ回。同じ対象のレビューと修正を繰り返す間、手順 3 で同じ箇所への指摘を数えるため。
   - 指摘を直すときの規則:
