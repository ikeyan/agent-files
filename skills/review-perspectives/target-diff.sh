...
# 出力 (stdout、1 行 1 つ): work=<レビュー対象ごとの作業ディレクトリ> run=<この実行の生成物のディレクトリ ($TMPDIR の下。消えて困るものは置かない)> repo=<レビュアーに渡すリポジトリのルート> diff=<target.diff> tree=<レビュー対象の内容全体 (未追跡を含む) の tree id> rules=<レビューの規則 (SKILL.md・観点・リポ固有の検索対象) の hash>
...
#   止まる:   同時に始めた別の実行と fetch が衝突した (cannot lock ref。やり直せば通る)、origin が無い、origin の HEAD が既定ブランチを指していない (set-head --auto の Cannot determine remote HEAD)、<対象> が解決できない (PR 番号で origin が GitHub のリポジトリでない・gh が認証されていないものを含む)、共通の祖先が無い、<path> がリポジトリの外 (絶対パスの .. でルートより上を通るものを含む)、レビュー対象が空 (変更が無い、<path> が何にも一致しない、コミットが打ち消し合って patch が空)
# 外さないもの: fetch の refspec と --prune、unshallow の origin、set-head --auto、--path-format=absolute --git-common-dir、add -A の前の read-tree、patch の diff-index、log の --no-show-signature (理由は canon の同ページ)
set -euo pipefail
...
# shallow だと PR の最初のコミットの親も、merge-base も first-parent の走査も途中で切れる (走査は失敗せず短い結果を返す)
...
if [ -z "$target" ]; then
  ...
  IFS=$'\t' read -r name base_branch first < <(gh pr view "$target" -R "$(git remote get-url origin)" --json headRefName,baseRefName,commits --jq '[.headRefName, .baseRefName, .commits[0].oid] | @tsv')
