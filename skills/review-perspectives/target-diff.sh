...
if [ -z "$target" ]; then
  ...
  IFS=$'\t' read -r name base_oid < <(gh pr view "$target" -R "$(git remote get-url origin)" --json headRefName,baseRefOid --jq '[.headRefName, .baseRefOid] | @tsv')
