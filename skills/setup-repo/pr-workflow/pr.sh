...
# [^/]+ は authority だけでなくクエリ・フラグメント・空白も通し、/graphql を続けると意図しないパスを叩く。ホスト名は RFC 1123 のラベル (英数字で始まり終わり、間にハイフン可、最大 63 文字) をドットでつなぎ、先頭・末尾のドットや連続するドットは許さない。IPv4 リテラルは構文上は数字だけのラベルの並びなので、範囲外の値 (300.1.1.1 など) もここは通り、resolve の失敗 (一時的) になる。スキームを省いた値は curl がホスト名として解釈して同じく resolve の失敗になるので、それもここで弾く
label='[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?'
[[ $api =~ ^https?://$label(\.$label)*(:([0-9]{1,5}))?$ ]] || { echo "$guard_msg" >&2; exit 2; }
# ポート 0 は curl の exit 7 (接続) になり一時的な失敗として再試行し続け、65536 以上は curl が起動時に拒否する。ドメインは 1〜65535 なので数値で確かめる (10#$port で先頭ゼロを 8 進数扱いさせない)。$label 自身がグループを持ち (\.$label)* で繰り返し展開されるので、ポートの数字は BASH_REMATCH[2] でなく [5] (実際に一致させて BASH_REMATCH を出し確認済み)
port=${BASH_REMATCH[5]}
