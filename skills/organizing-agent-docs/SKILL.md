...
| 外部依存 (CLI・ライブラリ・カーネル・API) の確定仕様 — 主語は常に外部依存側。repo の運用手順を処方として書かない (実測の再現に要る repo の文脈は evidence として残す) | canon の `facts/<topic>/` — 必要時に読む |
...
- **規律の正本は agent-files、事実の正本は canon** (`ikeyan/canon`): 汎用規律は agent-files で直し各 repo へ伝播する。外部依存の確定仕様と事故は repo を問わず canon に集める — 事実は repo に属さないので、同じ contract が repo ごとに分裂するのを防ぐ。repo 自身の構成・運用手順は README か対象コード近傍のコメントに置く (canon に混ぜると、外部仕様と自分の実装が同じ確度の「確定事実」に見えるうえ、他 repo には無関係な手順が混ざる)。
