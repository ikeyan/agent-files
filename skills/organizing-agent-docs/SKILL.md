...
| 外部依存の確定仕様 (主語は常に外部ツール — 自 repo の構成・手順は 1 行も載せない) | verified-facts ledger (`docs/verified-facts/<topic>.md`) — 必要時に読む |
...
- **正本は agent-files**: 汎用規律は agent-files で直し各 repo へ伝播する。その repo でだけ確かめた外部の事実は当該 repo の ledger に置き、repo 自身の構成・運用手順は README か対象コード近傍のコメントに置く (ledger に混ぜると、外部仕様と自分の実装が同じ確度の「確定事実」に見える)。
